...
/**
 * Records an error due to an incorrect parameter (``<param>``)
 * value. This is an error in the **schema** used to validate a
 * document. Note that these errors are *returned* by salve's
 * internal code. They are not *thrown*.
 *
 * @constructor
...
/**
 * Records an error due to an incorrect value (``<value>``).  This is
 * an error in the **schema** used to validate a document. Note that
...
/**
 * Records the failure of parsing a parameter (``<param>``)
 * value. Whereas {@link module:datatypes/errors~ParamError ParamError}
 * records each individual issue with a parameter's parsing, this
 * object is used to throw a single failure that collects all the
 * individual issues that were encountered.
 *
 * @constructor
...
/**
 * Records the failure of parsing a value (``<value>``). Whereas
 * {@link module:datatypes/errors~ValueError ValueError} records each
 * individual issue with a value's parsing, this object is used to
 * throw a single failure that collects all the individual issues that
 * were encountered.
 *
 * @constructor
