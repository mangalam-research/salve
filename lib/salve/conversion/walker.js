...
define(/** @lends module:conversion/walker */
  function walker(require, exports, _module) {
    "use strict";
    ...
    var nameToConstructor = formats.__protected.name_to_constructor;
    var constructors = [];
    var i = 0;
    while (nameToConstructor[i]) {
      constructors[i] = nameToConstructor[i];
      i++;
    }
     ...
     */
    ConversionWalker.prototype.openConstruct =
      function openConstruct(open, close) {
     ...
     * openConstruct}.
     */
    ConversionWalker.prototype.closeConstruct =
      function closeConstruct(close) {
     ...
     */
    ConversionWalker.prototype.outputKeyValue =
     ...
     */
    ConversionWalker.prototype.walkChildren =
      function walkChildren(el, startAt, endAt) {
        if (!startAt) {
          startAt = 0;
        }
    ...
    DatatypeProcessor.prototype.walk = function walk(el) {
      ...
      if (el.node.local === "value") {
        ...
        var params = [];
        _.each(el.children.slice(0, hasExcept ? el.children.length - 1 :
