...
ConversionWalker1.prototype.walk = function (el) {
    ...
    case "name":
        this.newItem();
        this.output.push('[' + this.ename_start);
    ...
    case "grammar":
        ...
        this.openConstruct("[", "]");
        this.newItem();
        this.output.push(this.verbose_format ? '"Grammar"' : constructor);
        if (this.include_paths)
            this.outputItem('"' + el.path + '"');
        this.walk(el.children[0]);
        this.newItem();
        this.openConstruct(this.array_start, ']');
    ...
    default:
        ...
        this.openConstruct("[", "]");
        this.newItem();
        this.output.push(this.verbose_format ? ('"' + capitalized  + '"') :
                         constructor);
        if (this.include_paths)
            this.outputItem('"' + el.path + '"');
        switch(node.local) {
