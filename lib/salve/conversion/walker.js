 ...
 * @static
 *
 * @property {Array.<string>} output The output of the conversion as an array of
 ...
 *
 * @param {string} close The closing string. This will be used to check that the
 * construct is closed properly.
 */
ConversionWalker.prototype.openConstruct = function openConstruct(open, close) {
  this._constructState.unshift({ open: open, close: close, first: true });
 ...
 * {@link module:conversion/walker.ConversionWalker#openConstruct
 * openConstruct}.
 */
ConversionWalker.prototype.closeConstruct = function closeConstruct(close) {
  var top = this._constructState.shift();
  if (close !== top.close) {
...
/**
 * Outputs a key-value pair. Both are strings. Outputs a separator (",") if this
 * is not the first item in the construct. The double-quotes in the strings will
 * be escaped and the strings will be surrounded by double quotes in the output.
 *
 * @param {string} key The key to output.
 * @param {string} value The value to output.
 */
ConversionWalker.prototype.outputKeyValue = function outputKeyValue(key, value) {
  this.newItem();
 ...
 *
 * @param {module:conversion/parser.Element} el The element whose children must
 * be walked.
 *
...
/**
 * @classdesc ConversionWalker for the default version generated by salve.
 * @class
 * @static
 *
...
DefaultConversionWalker.prototype.walk = function walk(el) {
  ...
  case "grammar":
    ...
    break;
  default:
    var capitalized = node.local.charAt(0).toUpperCase() + node.local.slice(1);
    ...
    case "data":
      ...
      var hasExcept = (el.children.length &&
...
/**
 * @classdesc A ConversionWalker specialized in gathering the names used for
 * Relax NG's ``<ref>`` and ``<define>`` elements.
 *
 * @class
 * @static
 *
 * @property {Object.<String,integer>} names The names gathered. Each name is
 * associated with the number of times it was seen. This property is valid after
 * the walker has walked the element tree.
 */
...
/**
 * @classdesc A ConversionWalker specialized in reassinging the names used by
 * Relax NG's ``<ref>`` and ``<define>`` elements.
 *
 * @class
 * @static
 *
 * @param {Object.<string, string>} names This is a map whose keys are the names
 * that already exist in the element tree and the values are the new names to
 * use. A ``(key, value)`` pair indicates that ``key`` should be replaced with
 * ``value``. It is up to the caller to ensure that two keys do not share the
 * same value and that the map is complete.
 */
...
/**
 * @classdesc This walker checks that the types used in the tree can be used,
 * and does special processing for QName and NOTATION.
 *
 * @class
 * @static
 *
 * @extends module:conversion/walker.ConversionWalker
 *
 * @property {Object} incompleteTypesUsed An anonymous object whose keys are the
 * types that have been deemed incomplete. This object is populated while
 * walking walk.
 *
 * @property {Array.<string>} warnings The warnings generated during the
 * walk. This array is populated while walking.
 */
function DatatypeProcessor() {
  ConversionWalker.call(this);
  this.warnings = [];
  this.incompleteTypesUsed = Object.create(null);
}
 ...
 *
 * @returns {string|undefined} The value of the attribute on the closest element
...
DatatypeProcessor.prototype.walk = function walk(el) {
  ...
  if (el.node.local === "value") {
    ...
    var params = el.children.slice(
      0, hasExcept ? el.children.length - 1 : undefined).map(
        function map(child) {
          return { name: child.node.attributes.name.value,
  ...
  if (libname === "http://www.w3.org/2001/XMLSchema-datatypes" &&
    ...
    this.warnings.push("WARNING: " + el.path + " uses the " + type +
                       " type in library " + libname);
