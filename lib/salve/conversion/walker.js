...
oop.inherit(ConversionWalkerV2, ConversionWalker);

/**
 * Open an array in the output.
 */
ConversionWalkerV2.prototype.openArray = function () {
    this.openConstruct("[", "]");
    this.outputItem(this.array_start);
};

ConversionWalkerV2.prototype.walk = function (el) {
    el.makePath();
    ...
    case "grammar":
        this.openConstruct("{", "}");
        this.outputItem('"v":2,"o":' + (this.include_paths ? 0 : 1) +
                        ',"d":');
...
DatatypeProcessor.prototype.walk = function(el) {
    ...
    if (el.node.local === "value") {
        ...
        if (has_except)
            throw new Error("except is not supported");
...
exports.ConversionWalkerV2 = ConversionWalkerV2;
