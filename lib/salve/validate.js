...
/**
 *
 * @param {module:validate~EName} other The other object to compare this object
 * with.
 *
 ...
 * passed to constructTree. These patterns form a JS representation of
 ...
 * @private
 *
 * @param {module:validate~Pattern} obj Object into which we must copy the
 ...
 * @private
 *
 * @returns {module:validate~Pattern} The pattern itself, if after transformation
 ...
 * meaningless so _cleanAttrs would modify the tree to replace the
 * group with its child. Supposing that <code>a</code> and <code>c</code> are attributes, <code>b</code>
 * is an element:
 *
 * <code>Group(Group(a, b), c) -> Group(a, c)</code>
 *
 * This is a destructive operation: it modifies the object on
 * which it is called. Expected to be called on patterns that have
 * been cloned from the original tree.
 ...
 * @private
 *
 * @returns {Array} A report of the form <code>[el, flag]</code>. The <code>el</code>
 * element is the pattern which replaces the pattern on which the
 * method was called. The <code>flag</code> element tells whether there has
 * been a modification inside <code>el</code>. The return value is undefined
 ...
 *
 *
 * @returns {module:validate~EName} The list of names used in the error
 * message.
 */
 ...
 * getNames, with each name replaced with a corresponding string.
 *
 ...
 * @constructor
 *
 * @param {Array.<module:validate~EName>} namea The name of the first XML entities at stake.
 * @param {Array.<module:validate~EName>} nameb The name of the second XML entities at stake.
 *
 */
 ...
 * call <code>(new Event(a, b, ...))</code> or the first call parameter may be a
 * list containing all the event parameters <code>(new Event([a, b, ])</code>. All
 ...
 * {@link module:hashstructs~HashSet HashSet} or a {@link module:hashstructs~HashMap HashMap}.
 *
...
/**
 * Utility function used mainly in testing to transform a {@link module:set~Set Set} of
...
/**
 * <p>Roughly speaking each {@link module:validate~Pattern Pattern} object has a corresponding Walker
 ...
 * <code>undefined</code> if no walker matches the
 * pattern. Otherwise, an array of {@link module:validate~ValidationError ValidationError} objects.
 */
 ...
 * error. Otherwise, a list of {@link module:validate~ValidationError ValidationError} objects.
 */
 ...
 * begins. For instance, if a Walker is processing <code>&lt;foo a="1"></code>,
...
/**
 * Mixin designed to be used for {@link module:validate~Walker Walkers} that can only have one
 * subwalker.
 *
 * @constructor
 * @private
...
/**
 * Mixin designed to be used for {@link module:validate~Walker Walkers} that cannot have any
 * subwalkers.
 *
 * @constructor
 * @private
...
ChoiceWalker.prototype.end = function () {
    ...
    // If we get here, we were not able to rase a ChoiceError,
...
GroupWalker.prototype.fireEvent = function(ev) {
    ...
    // must end walker_a
    if (!ev.isAttributeEvent() && ret_b !== undefined) {
 ...
 * @param {String} xml_path This is a string which uniquely identifies
 * the element from the simplified RNG tree. Used in debugging.
 * @param {module:validate~Pattern} start The start pattern of this grammar.
 * @param {Array} definitions An array of {@link module:validate~Pattern Pattern} objects which
 * contain all definitions specified in this grammar.
 *
 * @throws {module:validate~ReferenceError} When any definition in the original schema
...
GrammarWalker.prototype.useNameResolver = function() {
    if (this._name_resolver)
        throw new Erro("calle useNameResolver twice on the same walker");
...
GrammarWalker.prototype._suppressAttributes = function () { throw new Error("_suppressAttributes cannot be called on a GrammarWalker"); };
...
var name_to_constructor = {
    // Array = 0 is hardcoded elsewhere in the conversion code so don't
    // change it.
    0: Array,
    Empty: Empty,
    1: Empty,
...
//
function resolveArrayV1(arr, options) {
    if (arr[0] !== 0)
        throw new Error("array type not 0, but " + arr[0] + " for array "
                        + arr);
    ...
    for (var el_ix = 1, el; (el = arr[el_ix]) !== undefined; el_ix++) {
        if (el instanceof Array) {
            if (el[0] !== 0)
                ret.push(constructObjectV1(el, options));
            else
                ret.push(resolveArrayV1(el, options));
        }
        else
            ret.push(el);
    }
...
// MODIFICATIONS TO THIS FUNCTION MUST BE REFLECTED IN rng-to-js.xsl
//
function constructObjectV1(array, options) {
    if (array.length < 1)
    ...
    if (ctor === Array)
        throw new Error("trying to build array with constructObjectV1");
    ...
    if (array.length > 1) {
        ...
        if (add_path)
            args.unshift("");
        args.unshift(0);
        args = resolveArrayV1(args, options);
    }
    else if (add_path)
        args = [""];
    else
...
function constructTree(code) {
    ...
    if (typeof(parsed) === 'object' && !parsed.v) {
        ...
        if (version === 1)
            return constructObjectV1(parsed.d, options);
        else
