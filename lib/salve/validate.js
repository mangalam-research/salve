...
(function () {
    ...
    call_dump = function (msg, name, me) {
        trace(buf + msg + name + " on class " + me.constructor.name + " id " +
              me.id +
 ...
 * copy mappings. See _clone() above.
 */
 ...
 * @private
 *
 * @returns {module:validate~Pattern} The pattern itself, if after
 ...
 *
 * This is a destructive operation: it modifies the object on which it
 * is called. Expected to be called on patterns that have been cloned
 * from the original tree.
 *
 * @private
 *
 * @returns {Array} A report of the form <code>[el, flag]</code>. The
 * <code>el</code> element is the pattern which replaces the pattern
 * on which the method was called. The <code>flag</code> element tells
 * whether there has been a modification inside <code>el</code>. The
 ...
 *
 *
 * @returns {module:validate~EName} The list of names used in the
 * error message.
 */
 ...
 * @constructor
 *
 * @param {Array.<module:validate~EName>} namea The name of the first
 * XML entities at stake.
 * @param {Array.<module:validate~EName>} nameb The name of the second
 * XML entities at stake.
 *
 */
 ...
 * module:hashstructs~HashMap HashMap}.
 *
 ...
 * grammar.
 * @param {Array} definitions An array of {@link
 * module:validate~Pattern Pattern} objects which contain all
