 ...
 * when called with an object that a <code>HashBase</code> instance uses. Note
 * that it is a valid implementation strategy for the hash function to know how
 * to handle only a certain type of object, and not everything under the
 * sun. This entails that a <code>HashBase</code> object using this hash
 * function can only contain objects of the type that the hash function knows
 * how to handle.
 *
 ...
 * <strong>change</strong> such mapping.
 *
 * @private
 *
 ...
 * object.
 *
 * @throws {Error} If <code>s</code> is not of the same type as this object.
 */
 ...
 * value. If the value happens to be an <code>Array</code> then the function is
 * <emph>applied</emph> to this array. A return value which is truthy includes
 * the value, otherwise the value is excluded.  @returns An object of the same
 * class as the object on which the method is called. This object contains only
 * the value selected by the function.
 */
 ...
 *
 * @param {Function} hashF A function which returns a uniquely identifying hash
 * when called with an object that a <code>HashSet</code> instance uses. Note
 * that it is a valid implementation strategy for the hash function to know how
 * to handle only a certain type of object, and not everything under the
 * sun. This entails that a <code>HashSet</code> object using this hash function
 * can only contain objects of the type that the hash function knows how to
 * handle.
 *
 * @param {Object} [initial] An initial value for the object being constructed.
 *
 ...
 *
 * @param {Function} hashF A function which returns a uniquely identifying hash
 * when called with an object that a <code>HashMap</code> instance uses. Note
 * that it is a valid implementation strategy for the hash function to know how
 * to handle only a certain type of object, and not everything under the
 * sun. This entails that a <code>HashMap</code> object using this hash function
 * can only contain objects of the type that the hash function knows how to
 * handle.
 *
 * @param {Object} [initial] An initial value for the object being constructed.
