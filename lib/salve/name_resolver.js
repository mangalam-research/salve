 ...
 * is used so that {@link module:nameResolver~NameResolver#hash hash} can return
 * unique values.
 *
 * @private
 */
...
/**
 * <p>This method is called to indicate the start of a new context.  Contexts
 ...
 * context, otherwise it would merely overwrite the old definition.</p>
 *
 * <p>At creation, a <code>NameResolver</code> has a default context already
 * created. There is no need to create it and it is not possible to leave
 * it.</p>
 */
 ...
 * {@link module:nameResolver~NameResolver.enterContext enterContext}.
 */
 ...
 * name optionally prefixed by a namespace prefix. For instance, in
 * <code>&lt;html xml:lang="en"></code>, "html" is a name without a prefix, and
 * "xml:lang" is a name with the "xml" prefix. An expanded name is a (URI, name)
 * pair.
 *
...
/**
 * <p>Unresolves an expanded name to a qualified name. An expanded name is a
 ...
 * </pre>
 *
 * then <code>qname === qname2</code> is not necessarily true. This would happen
 * if two prefixes map to the same URI. In such case the prefix provided in the
 * return value is arbitrarily chosen.</p>
 *
