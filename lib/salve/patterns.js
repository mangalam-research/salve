 ...
 * @returns
 * {false|undefined|PartialMatch|Array.<module:patterns~ValidationError>}
...
EmptyWalker.prototype.fireEvent = function (ev) {
    if ((ev.params[0] === "text" || ev.params[0] === "attributeValue") &&
        (ev.params[1].trim() === ""))
        return false;
...
addWalker(Value, ValueWalker); // Cheat until we have a real Data library.
...
function ValueWalker(el) {
    Walker.call(this);
    this.el = el;
    // If we are supposed to match the empty string, we're ready to
    // end.
    this.matched = false;
    this.possible_cached = new EventSet(new Event("text", el.value));
}
...
ValueWalker.prototype.fireEvent = function(ev) {
    ...
    if (ev.params[0] !== "text" &&
        ev.params[0] !== "attributeValue")
        return undefined;

    var match = this.el.datatype.match(ev.params[1], this.el.value);
    if (!match)
        return undefined;

    switch(match.type) {
    case datatypes.MATCH:
        this.matched = true;
        this.possible_cached = new EventSet();
        if (match.length === undefined)
            return false; // Complete match.
        else
            return new PartialMatch(match.length);
        break;
    case datatypes.INCOMPLETE:
        return undefined;
    }
...
ValueWalker.prototype.canEnd = function () {
    return this.matched || this.el.value === "";
};
...
ValueWalker.prototype.end = function () {
    ...
    return [new ValidationError("value required: " + this.el.value)];
};
...
TextWalker.prototype.fireEvent = function (ev) {
    return  (ev.params[0] == "text" ||
             ev.params[0] === "attributeValue") ? false:
        undefined;
};
...
AttributeWalker.prototype._possible = function () {
    ...
    if (!this.seen_name)
        ...
        if (this.subwalker === undefined)
            this.subwalker = this.el.pat.newWalker();
