...
define(/** @lends module:patterns */
  function patterns(require, exports, _module) {
    "use strict";
    ...
    if (DEBUG) {
      ...
      (function () {
        ...
        var name_or_path = function name_or_path(walker) {
          var el = walker.el;
        ...
        call_dump = function call_dump(msg, name, me) {
          trace(buf + msg + name + " on class " + me.constructor.name +
                " id " + me.id + name_or_path(me));
        };

        possible_tracer = function possible_tracer(old_method, name, args) {
          buf += step;
        ...
        fireEvent_tracer = function fireEvent_tracer(old_method, name, args) {
          buf += step;
          call_dump("calling ", name, this);
        ...
        plain_tracer = function plain_tracer(old_method, name, args) {
          buf += step;
          call_dump("calling ", name, this);
    ...
    function addWalker(el_cls, walker_cls) {
      el_cls.prototype.newWalker = function newWalker(name_resolver) {
        /* jshint newcap: false */
     ...
     * @constructor
     *
     * @param {string} xml_path This is a string which uniquely identifies the
     ...
     * created.
     * @param {module:name_resolver~NameResolver} name_resolver Ignored by this
     * walker.
     */
    ...
    Ref.prototype.newWalker = function newWalker(name_resolver) {
    ...
    OneOrMoreWalker.prototype._instantiateNextIteration =
      function _instantiateNextIteration() {
        if (this.next_iteration === undefined) {
          ...
          // returns, the same is not true of next_iteration. So if we create it
    ...
    ChoiceWalker.prototype.fireEvent = function fireEvent(ev) {
      ...
      if (ret_b !== undefined) {
        this.chosen = true;
        // We do not need to test if ret_a is undefined because we would not
    ...
    GroupWalker.prototype._possible = function _possible() {
      ...
      if (this.suppressed_attributes) {
        ...
        if ((!this.ended_a || this.hit_b) && !this.walker_a.canEnd()) {
          ...
          possible_b = possible_b.filter(function filter(x) {
    ...
    GroupWalker.prototype.fireEvent = function fireEvent(ev) {
      ...
      if (!this.ended_a) {
        ...
        // We must return right away if walker_a cannot yet end. Only attribute
      ...
      // walker_a, if we've not already done so.
      if (!ev.isAttributeEvent() && ret_b !== undefined && !this.ended_a) {
        ...
        if (!ret_b) {
          // ret_b must be false, because ret_b === undefined has been
    ...
    Element.prototype.newWalker = function newWalker(name_resolver) {
    ...
    ElementWalker.prototype._possible = function _possible() {
      if (!this.seen_name) {
        ...
        var ret = new EventSet();
        // We use value_evs to record whether an attributeValue is a
    ...
    function DefineWalker(el, name_resolver) {
      ...
      this.subwalker = (el !== undefined) ?
        el.pat.newWalker(this.name_resolver) : undefined;
    }
    ...
    GrammarWalker.prototype.fireEvent = function fireEvent(ev) {
      ...
      if (ret instanceof PartialMatch) {
        ...
        switch (ev.params[0]) {
        case "enterStartTag":
          var name = new name_patterns.Name("", ev.params[1], ev.params[2]);
