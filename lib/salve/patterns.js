...
if (DEBUG) {
  ...
  /**
   * Utility function for debugging. Wraps <code>me[name]</code> in a wrapper
   * function. <code>me[name]</code> must be a function.  <code>me</code> could
   * be an instance or could be a prototype. This function cannot trivially wrap
   * the same field on the same object twice.
   *
   * @private
...
/**
 * <p>This method is mainly used to be able to use Event objects in a {@link
...
/**
 * <p>This method must be called after resolution has been performed.  _prepare
 * recursively calls children but does not traverse ref-define boundaries to
 * avoid infinite regress...</p>
 *
 ...
 * there is one and only one of each event created.</p>
 *
 * <p>An event is made of a list of event parameters, with the first one being
 * the type of the event and the rest of the list varying depending on this
 * type.</p>
 *
 * @constructor
 *
 * @param args... The event parameters may be passed directly in the call
 * <code>(new Event(a, b, ...))</code> or the first call parameter may be a list
 * containing all the event parameters <code>(new Event([a, b, ])</code>. All of
 * the event parameters must be strings.
 */
function Event() {
  var params;
...
/**
 * <p>This method is mainly used to be able to use Walker objects in a {@link
 * module:hashstructs~HashSet HashSet} or a {@link module:hashstructs~HashMap
 * HashMap}.</p>
 *
 * <p>Returns a hash guaranteed to be unique to this object. There are some
 ...
 * Array.<module:errors~ValidationError>} The value <code>false</code> if there
 * was no error. The value <code>undefined</code> if no walker matches the
 * pattern. A ``PartialMatch`` object if a chunk of text was partially
 * matched. (Note that this value is used only internally.)  Otherwise, an array
 ...
 * attributes, ``false`` otherwise.
 *
 * @returns {boolean|Array.<module:patterns~ValidationError>} <code>false</code>
 * if the walker ended without error. Otherwise, a list of {@link
 ...
 * tag begins. For instance, if a Walker is processing <code>&lt;foo
 * a="1"></code>, as soon as the greater than symbol is encountered, attribute
 * events are no longer possible. This function informs the Walker of this fact.
 *
 * @private
 */
...
OneOrMoreWalker.prototype._instantiateNextIteration =
  function _instantiateNextIteration() {
    if (this.nextIteration === undefined) {
      ...
      // that current_iteration is always existing and its _suppressAttributes()
  ...
  if (!this.ended_start_tag) {
    if (!this.seen_name) {
      if (ev.params[0] === "enterStartTag" &&
        ...
        this.seen_name = true;
        this.bound_name = new namePatterns.Name("", ev.params[1], ev.params[2]);
        this.end_tag_event = new Event("endTag", this.bound_name);
        return false;
      }
    }
...
/**
 * @classdesc <p>This is an exception raised to indicate references to undefined
 * entities in a schema. If for instance element A has element B as its children
 * but B is not defined, then this exception would be raised.</p>
 *
 * <p>This exception is indicative of an internal error because by the time this
 * module loads a schema, the schema should have been simplified already and
 * simplification should have failed due to the unresolvable reference.</p>
 *
 * @extends Error
 * @constructor
 *
 ...
 * attribute.
 *
 * @returns {module:patterns~EName|undefined} An expanded name, or undefined if
 ...
 * module:patterns~ValidationError ValidationError} objects.  @throws {Error}
 * When name resolving events (<code>enterContext</code>,
 * <code>leaveContext</code>, or <code>definePrefix</code>) are passed while
...
GrammarWalker.prototype.fireEvent = function fireEvent(ev) {
  ...
  if (ret instanceof PartialMatch) {
    ...
    case "text":
      ...
      break;
    case "leaveStartTag":
      // If the _misplaced_elements stack did not exist then we would get here
